package coursera.course1.week3.objsets
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList = {
    ...
    def loop(src: TweetSet, dest: TweetList): TweetList =
      if (src.isEmpty) dest
      else {
        val t = src.mostRetweeted
        loop(src.remove(t), new Cons(head = t, tail = dest))
      }
