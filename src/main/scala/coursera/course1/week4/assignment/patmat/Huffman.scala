package coursera.course1.week4.assignment.patmat
...
object Huffman extends App {
  ...
  def makeString(tree: CodeTree): String = tree match {
    case Fork(left, right, chars, weight) => makeString(left) + "<-[" + chars.mkString("") + ":" + weight + "]->" + makeString(right)
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    def traverse(list: List[(Char, List[Bit])]): List[Bit] =
      if (list.isEmpty) List()
      else if (list.head._1 == char) list.head._2
  ...
  println(encoded)
  println(quickEncode(tree.head)(List('a', 'b', 'f')))
