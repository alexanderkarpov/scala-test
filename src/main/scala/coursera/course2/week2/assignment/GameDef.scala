...
trait GameDef {
  ...
  case class Pos(row: Int, col: Int) {
    /** The position obtained by changing the `row` (x) coordinate by `d` */
    def dx(d: Int) = copy(row = row + d)

    /** The position obtained by changing the `col` (y) coordinate by `d` */
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    for(n <- neighbors if n._1.isLegal) yield n
