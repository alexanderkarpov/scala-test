...
trait Solver extends GameDef {
  ...
  for (n <- neighbors if !explored.contains(n._1)) yield n
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moveList) #:: tail => {
      val nextExplored = explored + block
      val nextNeighbors = newNeighborsOnly(neighborsWithHistory(block, moveList), nextExplored)

      nextNeighbors ++ from(tail ++ nextNeighbors, nextExplored)
    }
  }
